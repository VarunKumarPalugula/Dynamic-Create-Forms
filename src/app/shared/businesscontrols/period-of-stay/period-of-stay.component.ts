...
export class PeriodOfStayComponent implements OnInit {
  ...
  openPeriodDetails(content, i) {
    this.index = i;
    this.addPeriodModal = this.modalService.open(content, { centered: false, keyboard: false, size: 'lg', backdrop: 'static' });
  }
  ...
  finalObj(i?) {
    return i ? this.commonService.fileData[this.applicantKey]['period of stay']['finalObj'][`period${i}`] : this.commonService.fileData[this.applicantKey]['period of stay']['finalObj'];
  }
