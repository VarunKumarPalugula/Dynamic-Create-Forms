...
ngOnInit(): void {
  ...
  if (Object.keys(this.finalObj()).length && this.finalObj(['passportList']) && this.finalObj(['passportList']).length) {
    this.getFile()
  }
}
...
deleteFiles(file) {
  ...
  this.clientService.deleteFile(file, this.token).subscribe(
    (res: any) => {
      this.spinner.hide();
      if (res.Status == 1) {
        delete this.commonService.fileData[this.applicantKey]['passport, visa or travel documents']['finalObj']['passportList'][file];
        this.closeModal();
