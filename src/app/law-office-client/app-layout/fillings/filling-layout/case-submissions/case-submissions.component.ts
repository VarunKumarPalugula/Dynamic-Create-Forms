...
export class CaseSubmissionsComponent implements OnInit {
  ...
  constructor(private filingService: ClientFilingsService, private toaster: ToastrService,private spinner: NgxSpinnerService) { 
  ...
  getCaseSubMissions() {
    this.spinner.show();
    this.filingService.getCaseSubs(this.orgId, this.filingId,this.filingType, this.token).subscribe(
      (res: any) => {
        this.spinner.hide();
